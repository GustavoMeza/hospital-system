// Returns a module to perform CRUD operations on roles
// Dependencies:
// - connector: A database connection pool
module.exports = (connector) => ({
    create: (obj) => {
        var sql = "INSERT INTO roles (name, status, original_id, created_at, created_by) VALUES (?, ?, ?, ?, ?);";
        var escapedValues = [obj.name, obj.status, obj.original_id, obj.created_at, obj.created_by];
    ...
    update: (obj) => {
        ...
        var escapedValues = [obj.name, obj.status, obj.original_id, obj.created_at, obj.created_by, obj.id];
        return connector.execute(sql, escapedValues);
    },
    delete: (id) => {
